# DESeq2 Cavitary Disease 0 vs 1
# E. Lamont 
# 2.25.26
...
my_pipeSummary2 <- my_pipeSummary %>%
  ...
  dplyr::select(SampleID, Replicate, PID, Cavitary_disease) %>%
  # replace_na(list(PTLD_12_mo = "unknown")) %>% # Replace the NA because the dds will break with NA present
  column_to_rownames("SampleID")

# Make sure these are the same
setdiff(colnames(my_RawReads_bc3), rownames(my_pipeSummary2))
...
dds <- DESeqDataSetFromMatrix(countData = my_RawReads_bc3,
                              ...
                              design = ~ Cavitary_disease)
dds$Replicate
dds$PID # 180
# colData(dds)
design(dds) # ~Cavitary_disease
levels(dds$Cavitary_disease)
# "0" "1"  
...
ddsColl$Cavitary_disease <- relevel(ddsColl$Cavitary_disease, ref = "0")
...
# [1] "Intercept"               "Cavitary_disease_1_vs_0"  

# See how many samples are in each condition
table(ddsColl_DEG$Cavitary_disease)
# 0  1 
# 22 61 

# Get the results
res <- results(ddsColl_DEG, name = "Cavitary_disease_1_vs_0")
res
...
res_CavitaryDisease.1vs0_df <- as.data.frame(res) %>% # Could use res or resLFC and see how it is different
...
res_CavitaryDisease.1vs0_df <- add_DE_columns_function(res_CavitaryDisease.1vs0_df, log2fold_cutoff = 1, padj_cutoff = 0.05)

# Save as csv
# write.csv(res_CavitaryDisease.1vs0_df, "Data/DESeq2_Results/res_CavitaryDisease.1vs0_df.csv")
...
my_df <- res_CavitaryDisease.1vs0_df
my_volcano <- my_df %>%
...
#        file = "CavitaryDisease.1vs0_v1.pdf",
