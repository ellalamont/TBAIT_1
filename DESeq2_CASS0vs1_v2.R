# DESeq2 CASS0 vs CASS 1 with low CASS0 PID removed
# E. Lamont 
# 2.24.26
...
# Remove all the CASS0 that are PID<170
my_pipeSummary3 <- my_pipeSummary2 %>% 
  mutate(PID_number = as.numeric(str_extract(PID, "(?<=PID_)\\d+"))) %>%
  filter(!(PID_number < 170 & CASS_Pos == 0))
# Now only 136 rows

# Now need to go back and remove things from the raw reads
my_RawReads_bc4 <- my_RawReads_bc3 %>%
  dplyr::select(all_of(rownames(my_pipeSummary3)))
# Now only 136 columnes


# Put the pipeSummary in the same order as the raw reads
my_pipeSummary4 <- my_pipeSummary3[colnames(my_RawReads_bc4), ]
...
stopifnot(all(colnames(my_RawReads_bc4) == rownames(my_pipeSummary4)))
stopifnot(all(rownames(my_pipeSummary4) == colnames(my_RawReads_bc4)))

# Make the DESeqDataSet object
dds <- DESeqDataSetFromMatrix(countData = my_RawReads_bc4,
                              colData = my_pipeSummary4,
                              design = ~ CASS_Pos)
dds$Replicate
dds$PID # 136
colData(dds)
...
ddsColl$PID # 61
my_pipeSummary4 %>%  pull(PID) %>% unique() # 61 here also!
...
ddsColl$CASS_Pos <- relevel(ddsColl$CASS_Pos, ref = "0")
...
res_CASS1vs0_df <- as.data.frame(res) %>% # Could use res or resLFC and see how it is different
...
# Save as csv
write.csv(res_CASS1vs0_df, "Data/DESeq2_Results/DESeq2_res_CASS1vs0_fewer_df.csv")
...
text_up <- res_CASS1vs0_df %>% filter(DE1 == "significant up") %>% nrow()
text_down <- res_CASS1vs0_df %>% filter(DE1 == "significant down") %>% nrow()
...
#        file = "CASS1vsCASS0_DESeq2_v2.pdf",
