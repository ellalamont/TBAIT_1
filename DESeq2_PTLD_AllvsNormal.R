# DESeq2 PTLD All PTLD vs normal
# E. Lamont 
# 2.6.26
...
dds$PID # 180
# colData(dds)
design(dds) # ~PTLD_binary
levels(dds$PTLD_binary)
# [1] "Normal"  "unknown" "yesPTLD"  
...
ddsColl$PTLD_binary <- relevel(ddsColl$PTLD_binary, ref = "Normal")
...
# [1] "Intercept"                     "PTLD_binary_unknown_vs_Normal" "PTLD_binary_yesPTLD_vs_Normal" 

# See how many samples are in each condition
table(ddsColl_DEG$PTLD_binary)
#  Normal unknown yesPTLD 
# 22      42      19 

# Get the results
res <- results(ddsColl_DEG, name = "PTLD_binary_yesPTLD_vs_Normal")
res
...
res_PTLD.yesPTLDvsNormal_df <- as.data.frame(res) %>% # Could use res or resLFC and see how it is different
...
res_PTLD.yesPTLDvsNormal_df <- add_DE_columns_function(res_PTLD.yesPTLDvsNormal_df, log2fold_cutoff = 1, padj_cutoff = 0.05)

# Save as csv
# write.csv(res_PTLD.yesPTLDvsNormal_df, "Data/DESeq2_Results/res_PTLD.yesPTLDvsNormal_df.csv")
...
my_df <- res_PTLD.yesPTLDvsNormal_df
my_volcano <- my_df %>%
...
#        file = "PTLD.yesPTLDvsNormal_v1.pdf",
