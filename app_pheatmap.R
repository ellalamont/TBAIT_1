...
my_plot_themes <- theme_bw() +
  ...
  theme(legend.position = "none",legend.text=element_text(size=12),
        ...
        axis.text.x = element_text(angle = 0, size=5, vjust=1, hjust=0.5),
        axis.title.y = element_text(size=12),
        axis.text.y = element_text(size=12), 
        plot.subtitle = element_text(size=12))

my_annotation_colors <- list(
  PTLD_12_mo = c("H37Rv" = "#999999",
            "Both" = "#bc5300", 
            "Restrictive" = "#64c7ff",
            "Normal" = "#3c8b22", 
            "Obstructive" = "#964f96")
...
my_tpm2 <- my_tpm_bc # %>% dplyr::select(-contains("THP1"))

my_pipeSummary2 <- my_pipeSummary %>%
  filter(!is.na(PTLD_12_mo) | Type == "H37Rv") %>%
  dplyr::select(SampleID, PTLD_12_mo) %>%
  mutate(PTLD_12_mo = ifelse(is.na(PTLD_12_mo), "H37Rv", PTLD_12_mo)) %>% 
  column_to_rownames("SampleID")

# Filter TPM
my_tpm3 <- my_tpm2[, colnames(my_tpm2) %in% rownames(my_pipeSummary2)]

# Reorder annotation rows to match columns of tpm file
my_pipeSummary3 <- my_pipeSummary2[colnames(my_tpm3), , drop = FALSE]

# Define UI ----
ui <- fluidPage(
  titlePanel("TBAIT PTLD Pheatmap"),
  ...
  fluidRow(
    ...
    column(width = 4,
           ...
           checkboxGroupInput("conditions",
                              ...
                              choices = unique(my_pipeSummary3$PTLD_12_mo),
                              selected = unique(my_pipeSummary3$PTLD_12_mo)),
...
server <- function(input, output, session) {
  ...
  output$dynamic_pheatmap <- renderUI({
    ...
    num_genes <- sum(rownames(my_tpm3) %in% allGeneSetList[[input$my_GeneSetSource]][[input$my_GeneSet]])
  ...
  output$pheatmap <- renderPlot({
    ...
    timepoint_columns <- my_pipeSummary3 %>%
      mutate(SampleID = rownames(my_pipeSummary3)) %>%
      group_by(PTLD_12_mo) %>%
      summarise(
        Samples = list(
          intersect(SampleID, colnames(my_tpm3))
        ),
        .groups = "drop"
      ) %>%
      { setNames(.$Samples, .$PTLD_12_mo) }
    
    selected_genes <- get_selected_genes()
    
    # Filter data based on selected genes
    my_data <- my_tpm3[rownames(my_tpm3) %in% selected_genes, , drop = FALSE]
    
    # Now filter columns based on which conditions are checked
    if (!is.null(input$conditions)) {
      selected_cols <- unlist(timepoint_columns[input$conditions])
    ...
    if (ncol(my_data) == 0) {
      showNotification("No columns match the selected conditions.", type = "error")
      return(NULL)
    }
    ...
    p <- pheatmap(my_data, 
                  annotation_col = my_pipeSummary3, 
