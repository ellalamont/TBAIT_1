...
source("Import_DEG_sets.R") # for DESeq2_dfs
  ...
  output$volcano_plot <- renderPlotly({
    ...
    my_volcano <- plot_data %>%
      ...
      { if(input$show_gene_set) 
        geom_point(data = gene_set, color = "yellow", aes(col = .data[[de_col]], label = .data[[label_col]], text = gene))
        else 
          NULL } +
      geom_point(data = single_gene, color = "yellow", aes(col = .data[[de_col]], label = .data[[label_col]], text = gene)) + 
