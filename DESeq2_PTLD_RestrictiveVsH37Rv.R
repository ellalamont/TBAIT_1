# DESeq2 PTLD Restrictive vs H37Rv
# E. Lamont 
# 2.25.26
...
my_RawReads_bc2 <- my_RawReads_bc %>% 
  ...
  dplyr::select(!contains("pH_7")) # Only remove the pH7 Rvs because comparing to them now

# Remove genes with <5 reads
keep <- rowSums(my_RawReads_bc2 > 5) >= 0.5 * ncol(my_RawReads_bc2)
my_RawReads_bc3 <- my_RawReads_bc2[keep, ] # now only 4441 genes (instead of 4499)
...
my_pipeSummary2 <- my_pipeSummary %>%
  ...
  mutate(PTLD_binary = ifelse(PTLD_12_mo %in% c("Both", "Restrictive", "Obstructive"), "yesPTLD", PTLD_12_mo)) %>%
  mutate(PTLD_binary = ifelse(PID == "H37Rv", "H37Rv", PTLD_binary)) %>% 
  mutate(PTLD_12_mo = ifelse(PID == "H37Rv", "H37Rv", PTLD_12_mo)) %>% 
  mutate(PID = ifelse(PID == "H37Rv", paste0(PID, "_", row_number()), PID)) %>% # Need to do this so the H37Rv don't collapse as technical replicates
...
# [1] "Both"        "H37Rv"       "Normal"      "Obstructive" "Restrictive" "unknown"   
...
ddsColl$PID # 86 (includes 4 H37Rv that should not be collapsed together)
...
ddsColl$PTLD_12_mo <- relevel(ddsColl$PTLD_12_mo, ref = "H37Rv")
...
# [1] "Intercept"                       "PTLD_12_mo_Both_vs_H37Rv"       
# [3] "PTLD_12_mo_Normal_vs_H37Rv"      "PTLD_12_mo_Obstructive_vs_H37Rv"
# [5] "PTLD_12_mo_Restrictive_vs_H37Rv" "PTLD_12_mo_unknown_vs_H37Rv"    

# See how many samples are in each condition
table(ddsColl_DEG$PTLD_12_mo)
# H37Rv        Both      Normal Obstructive Restrictive     unknown 
# 4           3          21           2          14          42 

# Get the results
res <- results(ddsColl_DEG, name = "PTLD_12_mo_Restrictive_vs_H37Rv")
res
...
res_PTLD.RestrictiveVsH37Rv_df <- as.data.frame(res) %>% # Could use res or resLFC and see how it is different
...
res_PTLD.RestrictiveVsH37Rv_df <- add_DE_columns_function(res_PTLD.RestrictiveVsH37Rv_df, log2fold_cutoff = 1, padj_cutoff = 0.05)

# Save as csv
# write.csv(res_PTLD.RestrictiveVsH37Rv_df, "Data/DESeq2_Results/res_PTLD.RestrictiveVsH37Rv_df.csv")
...
text_up <- my_df %>% filter(DE1 == "significant up") %>% nrow()
text_down <- my_df %>% filter(DE1 == "significant down") %>% nrow()
...
#        file = "PTLD.RestrictiveVsH37Rv_v1.pdf",
